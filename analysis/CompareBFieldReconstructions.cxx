...
void MakeRhoPlot(TCanvas *c, TProfile *p_p_o, TProfile *p_m_o, TProfile *p_p_n, TProfile *p_m_n,
                  string oldMapName, string newMapName, bool print = false, string file_prefix="") {
  gStyle->SetHatchesSpacing(1.5);
  string name = p_p_o->GetName();
  name = name.substr(11,2);

  double maxt = 0.25;
  double mint = -0.25;
  
  p_p_o->SetTitle(Form(";#eta_{ID};#rho_{%s} (#mu^{+})",name.c_str()));
  ...
  p_m_o->SetTitle(Form(";#eta_{ID};#rho_{%s} (#mu^{-})",name.c_str()));
  ...
  if (print) c->Print(Form("plots/%srho_%s_%s_vs_%s.png", file_prefix.c_str(), name.c_str(), oldMapName.c_str(), newMapName.c_str()));
  else c->WaitPrimitive(); 
...
void MakePlots(bool print = false) {
  gStyle->SetOptStat(0);
  ...
  std::string file_prefix = "00339396.recon.DRAW_ZMUMU.stefano_";
  std::cout << "Preparing output with prefix " << file_prefix.c_str() << std::endl;
  ...
  //p_rho_plus_ME_o->BuildOptions(0,0,"s");
  ...
  TFile* f_recoOld = new TFile("ntuples_muonSelection/bfield_map_stefano_RunI.outputs.root");
  ...
  TFile* f_recoNew = new TFile("ntuples_muonSelection/group.perf-muons.15624486.2016.ANALYSIS.merged.root");
  ...
  // canvas for raw
  TCanvas *rawCan = new TCanvas("rawCan","",1000,1000);
  MakeRhoPlot(rawCan, p_rho_plus_ME_o, p_rho_min_ME_o, p_rho_plus_ME_n, p_rho_min_ME_n,
                 oldMapName, newMapName, print, file_prefix);
  MakeRhoPlot(rawCan, p_rho_plus_MS_o, p_rho_min_MS_o, p_rho_plus_MS_n, p_rho_min_MS_n,
                 oldMapName, newMapName, print, file_prefix);
  rawCan->Close();
