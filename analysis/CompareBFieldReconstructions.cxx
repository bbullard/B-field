...
void MakeRatioPlot(TCanvas *c, TH1F* h_o, TH1F* h_n, TLegend *l, TPaveText *p, 
  ...
  c->Print(Form("plots/%s%s_%s_vs_%s.png", file_prefix.c_str(), name.c_str(), oldMapName.c_str(), newMapName.c_str()));
  delete ratio;
...
void MakeProfile(TCanvas *c, TProfile *p_o, TProfile *p_n, string oldMapName, string newMapName, 
  ...
  TPaveText *p = new TPaveText(.43,.83,.66,.87,"NDC");
  p->AddText("#it{ATLAS} #bf{Internal}");
  p->SetFillColor(0);
  p->SetLineColor(0);
  p->SetBorderSize(0);
  ...
  c->Print(Form("plots/%s%s_%s_Profile_%s_vs_%s.png", file_prefix.c_str(), name.c_str(), type.c_str(), oldMapName.c_str(), newMapName.c_str()));
...
void MakeCSProfile(TCanvas *c, TProfile *p_p_o, TProfile *p_m_o, TProfile *p_p_n, TProfile *p_m_n,
  ...
  TPaveText *p = new TPaveText(.43,.87,.66,.97,"NDC");
  p->AddText("#it{ATLAS} #bf{Internal}");
  p->SetFillColor(0);
  p->SetLineColor(0);
  p->SetBorderSize(0);
  ...
  c->Print(Form("plots/%s%s_%s_ChargeSeparatedProfile_%s_vs_%s.png", file_prefix.c_str(), name.c_str(), type.c_str(), oldMapName.c_str(), newMapName.c_str()));
...
void MakeProfilePlots(string file_prefix, string oldFileName, string newFileName) {
  ...
  p_rho_plus_ME_o->SetMaximum(.25); 
  p_rho_plus_ME_o->SetMinimum(-.25); 
  ...
  // invariant mass
  TProfile* p_m_CB_o = new TProfile("p_m_CB_o","",nBins,-2.5,2.5,"s");
  p_m_CB_o->SetTitle(";#eta;m_{#mu#bar{#mu}} (GeV)");
  p_m_CB_o->SetMaximum(110);
  p_m_CB_o->SetMinimum(55); 
  TProfile* p_m_ME_o = (TProfile*)p_m_CB_o->Clone("p_m_ME_o");
  TProfile* p_m_MS_o = (TProfile*)p_m_CB_o->Clone("p_m_MS_o");
  TProfile* p_m_ID_o = (TProfile*)p_m_CB_o->Clone("p_m_ID_o");

  TProfile* p_m_CB_n = (TProfile*)p_m_CB_o->Clone("p_m_CB_n");
  TProfile* p_m_ME_n = (TProfile*)p_m_CB_o->Clone("p_m_ME_n");
  TProfile* p_m_MS_n = (TProfile*)p_m_CB_o->Clone("p_m_MS_n");
  TProfile* p_m_ID_n = (TProfile*)p_m_CB_o->Clone("p_m_ID_n");
  
  // pt
  // plus
  TProfile* p_pt_plus_CB_o = new TProfile("p_pt_plus_CB_o","",nBins,-2.5,2.5,"s");
  p_pt_plus_CB_o->SetTitle(";#eta;P_{T} (GeV)"); 
  p_pt_plus_CB_o->SetMaximum(0);
  p_pt_plus_CB_o->SetMinimum(120); 
  ...
  */
  ...
  vp_o->push_back(p_m_CB_o);
  vp_o->push_back(p_m_ME_o);
  vp_o->push_back(p_m_MS_o);
  vp_o->push_back(p_m_ID_o);
  ...
  vp_n->push_back(p_m_CB_n);
  vp_n->push_back(p_m_ME_n);
  vp_n->push_back(p_m_MS_n);
  vp_n->push_back(p_m_ID_n);
  ...
  if ( vh_plus_o->size() != vh_plus_n->size() || vh_plus_o->size() != vh_min_o->size()
        || vh_plus_o->size() != vh_min_n->size()) 
  ...
  while (reader_old.Next()) {
    // fill invariant mass
    if (*m_CB_o > 0) 
      p_m_CB_o->Fill(*m_CB_o);
    if (*m_ME_o > 0) 
      p_m_ME_o->Fill(*m_ME_o);
    if (*m_MS_o > 0) 
      p_m_MS_o->Fill(*m_MS_o);
    if (*m_ID_o > 0) 
      p_m_ID_o->Fill(*m_ID_o);
  
    // loop over two muons
    for (int i = 0; i < 2; i++) {
      // if CB muon
      if (pt_CB_o->at(i) != -1) {
        p_pt_CB_o->Fill(pt_CB_o->at(i));
        if (charge_CB_o->at(i) > 0) p_pt_plus_CB_o->Fill(pt_CB_o->at(i));
        if (charge_CB_o->at(i) < 0) p_pt_min_CB_o->Fill(pt_CB_o->at(i));
      }
  
      // if ME muon
      if (pt_ME_o->at(i) != -1) {
        p_pt_ME_o->Fill(pt_ME_o->at(i));
        if (charge_ME_o->at(i) > 0) p_pt_plus_ME_o->Fill(pt_ME_o->at(i));
        if (charge_ME_o->at(i) < 0) p_pt_min_ME_o->Fill(pt_ME_o->at(i));
      }

      // if MS muon
      if (pt_MS_o->at(i) != -1) {
        p_pt_MS_o->Fill(pt_MS_o->at(i));
        if (charge_MS_o->at(i) > 0) p_pt_plus_MS_o->Fill(pt_MS_o->at(i));
        if (charge_MS_o->at(i) < 0) p_pt_min_MS_o->Fill(pt_MS_o->at(i));
      }

      // if ID muon
      if (pt_ID_o->at(i) != -1) {
        p_pt_ID_o->Fill(pt_ID_o->at(i));
        if (charge_ID_o->at(i) > 0) p_pt_plus_ID_o->Fill(pt_ID_o->at(i));
        if (charge_ID_o->at(i) < 0) p_pt_min_ID_o->Fill(pt_ID_o->at(i));
  ...
  while (reader_new.Next()) {
    // fill invariant mass
    if (*m_CB_n > 0) 
      p_m_CB_n->Fill(*m_CB_n);
    if (*m_ME_n > 0) 
      p_m_ME_n->Fill(*m_ME_n);
    if (*m_MS_n > 0) 
      p_m_MS_n->Fill(*m_MS_n);
    if (*m_ID_n > 0) 
      p_m_ID_n->Fill(*m_ID_n);
  
    // loop over two muons
    for (int i = 0; i < 2; i++) {
      // if CB muon
      if (pt_CB_n->at(i) != -1) {
        p_pt_CB_n->Fill(pt_CB_n->at(i));
        if (charge_CB_n->at(i) > 0) p_pt_plus_CB_n->Fill(pt_CB_n->at(i));
        if (charge_CB_n->at(i) < 0) p_pt_min_CB_n->Fill(pt_CB_n->at(i));
      }
  
      // if ME muon
      if (pt_ME_n->at(i) != -1) {
        p_pt_ME_n->Fill(pt_ME_n->at(i));
        if (charge_ME_n->at(i) > 0) p_pt_plus_ME_n->Fill(pt_ME_n->at(i));
        if (charge_ME_n->at(i) < 0) p_pt_min_ME_n->Fill(pt_ME_n->at(i));
      }

      // if MS muon
      if (pt_MS_n->at(i) != -1) {
        p_pt_MS_n->Fill(pt_MS_n->at(i));
        if (charge_MS_n->at(i) > 0) p_pt_plus_MS_n->Fill(pt_MS_n->at(i));
        if (charge_MS_n->at(i) < 0) p_pt_min_MS_n->Fill(pt_MS_n->at(i));
      }

      // if ID muon
      if (pt_ID_n->at(i) != -1) {
        p_pt_ID_n->Fill(pt_ID_n->at(i));
        if (charge_ID_n->at(i) > 0) p_pt_plus_ID_n->Fill(pt_ID_n->at(i));
        if (charge_ID_n->at(i) < 0) p_pt_min_ID_n->Fill(pt_ID_n->at(i));
...
void GeneratePlots() {
  ...
  string oldFileName = "group.perf-muons.15646118.ANALYSIS.MERGED.sub1.root";
  string newFileName = "group.perf-muons.15646118.ANALYSIS.MERGED.sub2.root";
