...
BFieldSolenoid::getB( const double *xyz, double *B, double *deriv ) const
{
    ...
    if ( ! m_cache.inside( z, r, phi ) ) {
        // outside the last bin
        if ( m_tilt && m_tilt->inside( z, r, phi ) ) {
            m_tilt->getCache( z, r, phi, m_cache );
        } else {
            B[0] = B[1] = B[2] = 0.0;
        }
    }
    m_cache.getB( z, r, phi, B, deriv );
}
